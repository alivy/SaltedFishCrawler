...
namespace Utils.Help
{
    ...
    public class ValidateHelper
    {
        ...
        /// <returns></returns>
        public static bool IsInt(string numericStr)
        {
            if (string.IsNullOrEmpty(numericStr))
            {
                return false;
            }
            return _intregex.IsMatch(numericStr);
        }
        ...
        /// <param name="param">key:属性名称 value:类型(int、double)</param>
        /// <returns>0:成功 1:不存在属性 2:类型有误</returns>
        public static int ValidatePara(JObject data, Dictionary<string, string> param)
        {
            var isHas = false;
            ...
            {
                isHas = data.Properties().Any(p => p.Name == item.Key);
                if (!isHas)
                {
                    retValue = 1;
                    break;
                }
                strValue = data[item.Key].ToString();
                if (item.Value == "int")
                {
                    if (!ValidateHelper.IsInt(strValue))
                    {
                        retValue = 2;
                        break;
                    }
                ...
                {
                    if (!ValidateHelper.IsNumeric(strValue))
                    {
                        retValue = 2;
                        break;
                    }
                ...
                {
                    //验证字符串，防止SQL注入
                    if (ValidateHelper.CheckKeyWord(strValue))
                    {
                        retValue = 2;
                        break;
                    }
        ...
        /// <param name="param">key:属性名称 value:类型(int、double)</param>
        /// <returns>0:成功 1:不存在属性 2:类型有误</returns>
        public static int ValidateParaNull(JObject data, Dictionary<string, string> param)
        {
            var isHas = false;
            var strValue = string.Empty;
            var retValue = 0;
            ...
            {
                isHas = data.Properties().Any(p => p.Name == item.Key);
                if (isHas && !string.IsNullOrEmpty(data[item.Key].ToString()))
                {
                    //retValue = 1;
                    //break;
                    strValue = data[item.Key].ToString();
        ...
        {
            //过滤关键字
            string StrKeyWord = @"select|insert";// @"select|insert|delete|from|count\(|drop table|update|truncate|asc\(|mid\(|char\(|xp_cmdshell|exec master|netlocalgroup administrators|:|net user|""|or|and";
            //过滤关键字符
            string StrRegex = @"[******|********]";// @"[-|;|,|/|\(|\)|\[|\]|}|{|%|\@|*|!|']";
